...
class Review_model extends CI_Model
{
    ...
    function add_data($data){
        $row = $this->db->insert('testimonial',$data);
        print_r($row);
    }
    function get_data(){
        $collams = array(
...
//        limit and Other Stop
        $row = $this->db->get('testimonial');
        $data = $row->result_array();
        for($i=0;$i <$row->num_rows();$i++){

//$data[$i]['image'] = '<img src='.base_url('uploads/testimonial/'). $data[$i]['image'].' height="75">';

 if($data[$i]['image'] !='') {
                $data[$i]['image'] = '<img src=' . base_url('uploads/testimonial/') . $data[$i]['image'] . ' style="height: 75px;">';
            }
            else
            {
                $data[$i]['image'] = '<img src="'.base_url().'uploads/no_thumb.jpeg" style="height: 75px;">';
            }
            $data[$i]['cheked'] = '<label class="mt-checkbox mt-checkbox-single mt-checkbox-outline">
                                        <input type="checkbox" class="Checkbox checkboxes" value="'. $data[$i]['testimonial_id'] .'" name="multy[]" id="multy"/>
                                        <span></span></label>';
            $data[$i]['Actions'] = '<div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu pull-left" role="menu">
                                            <li>
                                                <a href="'. base_url('Reviews/edit/') . $data[$i]['testimonial_id'] .'">
                                                    <i class="icon-docs"></i>Edit</a>
                                            </li>
                                            <li>
                                                <a href="javascript:;" onclick="delete_review('. $data[$i]['testimonial_id'] .');">
    ...
    function delete($ids){
        $this->db->where(['testimonial_id'=>$ids]);
    ...
    function edit_review($new,$id){
        ...
        $this->db->where(['testimonial_id' => $id]);
        $this->db->update('testimonial  ',$new);
    }
